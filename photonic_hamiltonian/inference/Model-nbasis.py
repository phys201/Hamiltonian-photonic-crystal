...
def ham_func(ham_params):
    u11, u20, e0, de =  ham_params
    ex = e0+de
    ey = e0-de
    ham = np.array([[ex,u11,u20,u11],
                    [u11,ey,u11,u20],
                    [u20,u11,ey,u11],
                    [u11,u20,u11,ex]])
    return ham
...
def model(theta, x):
    ...
    theta (list of Floats or PyTensors):
        The list of fitting parameters, in the order [parameters for the Hamiltonian, A0, peak heights, peak widths]
    ...
    """
    #for our n-basis Hamiltonian, the parameters include
    #interaction-between-modes terms ;
    #background A0 and heights of n peaks A1, A2, ..., An
    #peak widths W1, W2, ..., Wn
    
    Wn_np = np.array(theta[-n:])
    An_np = np.array(theta[-2*n:-n])
    A0 = theta[-(2*n+1)]
    ham_params = theta[:-(2*n+1)]
...
#     ey = e0 - de   # for k = (+-0.05,0) the energy of uncoupled slab mode 2
    ...
    ham_np = ham_func(ham_params) #we will need to define a ham function that works for arbitrary number of basis
    ...
    if type(theta[0]) == float:
        ...
        line_each = [Ai * np.exp(-(freq - Ci)**2 / (2 * Wi**2)) for Ai, Ci, Wi in zip(An_np, Cn_np, Wn_np)]
        line = np.sum(line_each, axis=0) + A0
    else:
        ham = pytensor.shared(np.zeros((n,n)))
        for row in range(n):
            for col in range(n):
                ham = pt.set_subtensor(ham[row, col], ham_np[row, col])

        #peak heights, peak positions, and peak widths
        An = pytensor.shared(np.zeros(n))
        Wn = pytensor.shared(np.zeros(n))
        for col in range(n):
...
#         return line
    ...
    with ham_model:
        ...
        model_predictions = model(theta, freq)
